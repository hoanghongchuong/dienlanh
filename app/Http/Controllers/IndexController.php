...
class IndexController extends Controller {
	...
	*/
	...
	{		
        ...
        $products = Products::where('cate_id',$product_cate->id)->where('status',1)->paginate(20);       
            ...
            return view('templates.productlist_tpl', compact('products', 'product_cate', 'keyword', 'description', 'title', 'img_share', 'cate_parent', 'com'));
        } else {
	...
	public function getDichVuList($alias){
		$cate_service = DB::table('news_categories')->where('status',1)->where('com','dich-vu')->where('alias',$alias)->first();
		$array_id = [];
		$array_id[] = $cate_service->id;
		$cate_childs = DB::table('news_categories')->where('status',1)->where('com','dich-vu')->where('parent_id', $cate_service->id)->get();
		foreach ($cate_childs as $key => $value) {
			$array_id[] = $value->id;
		}
		$data = DB::table('news')->where('com', 'dich-vu')->where('status',1)->whereIn('cate_id', $array_id)->paginate(20);
		if(!empty($cate_service->title)){
			$title = $cate_service->title;
		}else{
			$title = $cate_service->name;
		}		
		$keyword = $cate_service->keyword;
		$description = $cate_service->description;
		return view('templates.cateservice_tpl', compact('cate_service', 'data', 'title','description', 'keyword'));
	}
	public function getDichVuDetail($alias)
	{
		$data = DB::table('news')->where('com', 'dich-vu')->where('status',1)->where('alias', $alias)->first();
		...
		return view('templates.detail_service', compact('data', 'title', 'description', 'keyword','items'));
	}
